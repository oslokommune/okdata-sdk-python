...
def test_create_webhook_token(requests_mock):
    ...
    def body_matcher(request):
        return request.json() == {"service": service_name}

    res = {
        "token": "774d8f35-fb4b-4e06-9d7f-54d1a08589b4",
        "created_by": "your-username",
        "dataset_id": dataset_id,
        "service": f"service-account-{service_name}",
    ...
    assert client.create_webhook_token(dataset_id, service_name) == res
...
def test_authorize_webhook_token(requests_mock):
    ...
    dataset_id = "dataset-id"
    token = "774d8f35-fb4b-4e06-9d7f-54d1a08589b4"
    matcher = re.compile(f"{client.api_url}/{dataset_id}/tokens/{token}/authorize")
    ...
    assert client.authorize_webhook_token(dataset_id, token) == res
