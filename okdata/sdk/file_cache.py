...
from okdata.sdk.io_utils import write_to_origo_cache, read_from_origo_cache
...
class FileCache:
    ...
    def write_credentials(self, credentials):
        if self.credentials_cache_enabled:
            ...
            write_to_origo_cache(content=str(credentials), filename=filename)
        else:
    ...
    def read_credentials(self):
        if self.credentials_cache_enabled:
            filename = f"client_credentials-{self.env}.json"
            credentials = read_from_origo_cache(filename=filename)
            if credentials:
                try:
